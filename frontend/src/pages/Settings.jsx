...
export default function Settings() {  
  ...
  // Recovery codes returned once by the backend (on enable / regenerate)  
  const [recoveryCodes, setRecoveryCodes] = useState(null);  
  const [recoveryModal, setRecoveryModal] = useState(false);  
  
  // Debounced password-strength lookup (shared hook).  
  ...
  const verify2FA = async () => {  
    try {  
      const res = await authService.verify2FA(otpToken);  
      toast.success("2FA activée avec succès");  
      setTwoFAModal(false);  
      setOtpToken("");  
      updateUser({ ...user, twoFactorEnabled: true });  
      // Show the backup recovery codes returned once by the backend  
  ...
  const regenerateRecoveryCodes = () => {  
    requireReauth(  
      ...
      {  
        ...
        description:  
          "Confirmez votre mot de passe. Vos anciens codes seront invalidés.",  
  ...
  const downloadRecoveryCodes = () => {  
    ...
    const blob = new Blob(  
      [  
        "Codes de récupération CredenVault\n" +  
    ...
    a.href = url;  
    a.download = "credenvault-recovery-codes.txt";  
    a.click();  
  ...
  const disable2FA = async () => {  
    try {  
      const payload = useRecoveryForDisable  
        ? {  
            password: disableForm.password,  
            recoveryCode: disableForm.recoveryCode,  
          }  
        : { password: disableForm.password, token: disableForm.token };  
      await authService.disable2FA(payload);  
      toast.success("2FA désactivée");  
      updateUser({ ...user, twoFactorEnabled: false });  
      setDisableForm({ password: "", token: "", recoveryCode: "" });  
  ...
  return (  
    <div className="space-y-6 animate-fade-in max-w-3xl">  
          ...
          <div className="space-y-4">  
            ...
            <div className="p-4 bg-slate-800/50 rounded-lg border border-slate-700 flex items-start gap-3">  
              ...
              <div className="flex-1 text-sm">  
                ...
                <p className="text-slate-400 mb-3">  
                  Utilisez un code de récupération pour vous connecter si vous  
                  perdez l'accès à votre application authenticateur. La  
                  régénération invalide tous les anciens codes.  
                </p>  
                <Button  
                  variant="secondary"  
            ...
            <div className="p-4 bg-slate-800/50 rounded-lg border border-slate-700">  
              ...
              <div className="space-y-3">  
                ...
                {useRecoveryForDisable ? (  
                  <Input  
                    ...
                    onChange={(e) =>  
                      setDisableForm((p) => ({  
                        ...p,  
                        recoveryCode: e.target.value.toUpperCase().slice(0, 20),  
                      }))  
                    }  
                ...
                <button  
                  type="button"  
                  onClick={() => setUseRecoveryForDisable((v) => !v)}  
                  className="text-xs text-brand-400 hover:underline"  
                >  
                  {useRecoveryForDisable  
                    ? "Utiliser plutôt un code OTP"  
                    : "J'ai perdu mon application authenticateur — utiliser un code de récupération"}  
                </button>  
                <Button  
                  variant="danger"  
                  onClick={disable2FA}  
        ...
        {twoFAData && (  
          <div className="space-y-5">  
            ...
            <p className="text-xs text-slate-500 text-center">  
              ⚠️ Vous recevrez vos codes de récupération après activation  
            </p>  
          </div>  
        )}  
      </Modal>  
  
      {/* Recovery codes display modal (shown once) */}  
      <Modal  
      ...
      >  
        {recoveryCodes && (  
          <div className="space-y-5">  
            <div className="bg-yellow-600/10 border border-yellow-600/30 rounded-lg p-4 text-sm text-yellow-500 flex items-start gap-3">  
              <AlertTriangle className="w-5 h-5 shrink-0 mt-0.5" />  
              <p>  
                Conservez ces codes en lieu sûr. Ils ne seront affichés  
                <strong> qu'une seule fois</strong>. Chaque code est utilisable  
                une seule fois pour vous connecter si vous perdez votre  
                application authenticateur.  
              </p>  
            </div>  
  
            <div className="grid grid-cols-2 gap-2 p-4 bg-slate-900 rounded-lg border border-slate-700 font-mono text-sm text-slate-100">  
              {recoveryCodes.map((code) => (  
                <span key={code} className="text-center py-1 tracking-wider">  
                  {code}  
                </span>  
              ))}  
            </div>  
