...
export default function Sidebar({ open, onClose }) {
  ...
  const navContent = (
    <>
      <div className="flex bg-blue-900 bg-opacity-5 items-center gap-2 px-4 py-6 border-b border-slate-800">
        ...
        <button onClick={onClose} className="ml-auto border border-slate-500 rounded-lg lg:hidden p-1 text-slate-400 hover:text-slate-100 transition-colors">
          <X className="w-5 h-5" />
        </button>
      </div>
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `flex items-center gap-4 px-3 py-3 rounded-xl text-sm font-medium transition-colors ${
                isActive
                  ? 'bg-brand-600/20 text-brand-400 border border-brand-500/20 border-l-8'
