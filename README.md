...
| Backend | Node.js, Express 4, Mongoose 8 (MongoDB), Zod, bcryptjs, jsonwebtoken, cookie-parser, speakeasy, qrcode, nodemailer, helmet, express-rate-limit |  
...
# Frontend URL used for email CTA links (optional; falls back to CORS_ORIGIN, then localhost)  
FRONTEND_URL=http://localhost:5173  
  
# Email (Nodemailer) — optional. Powers welcome, login-alert, password-changed,  
# and password-renewal emails. When unset, emails are simulated in the console.  
...
Protected routes are authenticated via the JWT httpOnly cookie, with an  
`Authorization: Bearer <token>` header accepted as a fallback. Base path: `/api`.  
...
- **Rate limiting** (as configured in `server.js`):  
  ...
  - Accounts (`/api/accounts`): 100 / min  
- **Other**: helmet headers, JSON body limit (10kb), CORS restricted to `CORS_ORIGIN` with credentials enabled, Zod validation on all mutating routes.  
